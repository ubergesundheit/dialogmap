...
(->
  ...
  tagsInput.run [
    ...
    ($templateCache) ->
      ...
      $templateCache.put "ngTagsInput/tags-input.html", "<div class=\"host\" tabindex=\"-1\" ti-transclude-append=\"\"><div class=\"tags\" ng-class=\"{focused: hasFocus}\"><ul class=\"tag-list\"><li class=\"tag-item\" ng-repeat=\"tag in tagList.items track by track(tag)\" ng-class=\"{ selected: tag == tagList.selected }\" lol-ng-click=\"tagList.selected = $index\"><span>{{getDisplayText(tag)}}</span> <a class=\"remove-button\" ng-click=\"tagList.remove($index)\">{{options.removeTagSymbol}}</a></li><li class=\"tag-item add_reference\"><span>+</span></li></ul></div></div>"
